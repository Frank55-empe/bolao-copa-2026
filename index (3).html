...
import { CheckCircle2, Share2, Copy, Trophy, Send, ArrowRight, MessageSquare, Ticket, AlertCircle } from 'lucide-react';
...
export default function Pix() {
  ...
  // ✅ CORREÇÃO 2: controle manual de confirmação de Pix
  ...
  const getWhatsAppLink = (recipientPhone: string, isAdmin: boolean) => {
    ...
    const message =
      `🏆 *BOLÃO MUNDIAL 2026* 🏆\n` +
      `-----------------------------------\n` +
      `*PALPITE REGISTRADO — AGUARDANDO PIX!*\n\n` +
      `👤 *Participante:* ${name}\n` +
      ...
      `🔑 *Chave Pix:* ${pixKey}\n\n` +
      `${isAdmin
        ? `👉 *Ação Requerida:* Olá admin, segue comprovante do meu palpite! Aguardo confirmação no painel. ⚽`
        : `⚠️ *Aviso:* Realize o pagamento do Pix para validar seu palpite. Guarde este comprovante!`}`;
    return `https://api.whatsapp.com/send?phone=${cleanRecipient}&text=${encodeURIComponent(message)}`;
  };

  // ✅ CORREÇÃO 2: sem auto-redirect — removido o useEffect que abria WhatsApp automaticamente
  ...
  const handleShare = () => {
    ...
    const text = encodeURIComponent(`Participe do Bolão Mundial 2026! Faça seus palpites nos maiores jogos da Copa. Acesse: ${url}`);
  ...
  // ✅ CORREÇÃO 2: tela de confirmação após clicar "Já fiz o Pix"
  if (pixConfirmed) {
    return (
      <div className="max-w-md w-full mx-auto pb-8 px-4">
        <motion.div
          ...
          className="glass-card border-2 border-[#009739]/50 rounded-3xl p-8 text-center relative overflow-hidden shadow-[0_0_50px_rgba(0,151,57,0.2)]"
        >
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            transition={{ delay: 0.2, type: "spring" }}
            className="w-20 h-20 bg-[#009739]/20 rounded-full flex items-center justify-center mx-auto mb-5 relative"
          >
          ...
          <h2 className="text-2xl font-display font-black tracking-widest text-[#FFCD00] mb-2">PALPITE ENVIADO!</h2>
          <p className="text-white/60 text-sm mb-8 leading-relaxed px-4">
            Seu palpite foi registrado e o Pix enviado. Aguarde a confirmação do administrador para validar sua participação no bolão. 🏆
          </p>

          <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4 mb-8 text-left">
            <p className="text-emerald-400 text-xs font-bold uppercase tracking-widest mb-1">O que acontece agora?</p>
            <p className="text-white/60 text-xs leading-relaxed">
              O administrador irá verificar seu pagamento Pix e confirmar sua participação. Você receberá uma mensagem no WhatsApp quando seu palpite for aprovado.
            </p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full bg-[#009739] text-white font-extrabold text-xs py-3.5 rounded-xl flex items-center justify-center gap-1.5 uppercase tracking-widest hover:bg-[#00702a] transition-all"
            >
              Ver Meus Palpites <ArrowRight className="w-3.5 h-3.5" />
            </button>
          </div>
        </motion.div>
      </div>
  ...
  return (
    <div className="max-w-md w-full mx-auto pb-8 px-4">
      ...
      >
        ...
        <h2 className="text-xl md:text-2xl font-display font-black tracking-widest text-[#FFCD00] mb-1">PALPITE REGISTRADO!</h2>
        <p className="text-white/60 text-xs mb-6 font-medium px-4">
          Agora realize o pagamento via Pix para garantir sua participação no bolão.
        </p>

        {/* ✅ Banner de instrução */}
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-3.5 mb-6 flex items-start gap-3 text-left">
          <AlertCircle className="w-5 h-5 text-[#FFCD00] shrink-0 mt-0.5" />
          <div>
            <p className="text-white text-[11px] font-black uppercase tracking-widest mb-0.5">Atenção — Passo a Passo</p>
            <p className="text-white/60 text-[11px] leading-relaxed">
              1. Copie a chave Pix abaixo<br />
              2. Realize o pagamento no seu banco<br />
              3. Clique em <strong className="text-[#FFCD00]">"Já fiz o Pix"</strong> para confirmar
            </p>
          </div>
        </div>

        {/* Comprovante do palpite */}
        {predictionState && (
          <motion.div
          ...
          >
            ...
            <div className="flex items-center justify-between bg-black/20 rounded-xl p-3 mb-3 border border-white/5">
              ...
              <span className="text-lg font-black text-[#FFCD00] px-3 bg-white/5 rounded-md border border-white/10">
                {predictionState.goalsA} x {predictionState.goalsB}
              </span>
        ...
        {/* ✅ CORREÇÃO 2: botão principal de confirmação */}
        <button
          onClick={() => setPixConfirmed(true)}
          className="w-full bg-gradient-to-r from-[#009739] to-[#00702a] text-white font-black uppercase tracking-widest py-4 rounded-xl flex items-center justify-center gap-2 transition-all hover:scale-[1.01] active:scale-95 text-sm shadow-lg mb-3"
        >
          <CheckCircle2 className="w-5 h-5" /> JÁ FIZ O PIX ✓
        </button>

        {/* Botões secundários de WhatsApp */}
        {predictionState && (
          <div className="space-y-2 mb-4 text-xs">
            <button
              onClick={openUserWhatsApp}
              className="w-full bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 text-emerald-400 font-bold uppercase tracking-wider py-3 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer text-[11px]"
            >
              <MessageSquare className="w-4 h-4" /> MEU COMPROVANTE NO WHATSAPP 💬
            </button>
            <button
              onClick={openAdminWhatsApp}
              className="w-full bg-white/5 border border-white/10 text-white/70 hover:bg-white/10 font-bold uppercase tracking-wider py-3 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer text-[10px]"
            >
            ...
            <div className="pt-2 text-center border-t border-white/5 mt-3">
              <p className="text-[9px] text-white/40 font-bold uppercase block mb-1.5">Número errado ou sem WhatsApp?</p>
              <div className="flex flex-col gap-1.5 items-center justify-center">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 bg-white/5 hover:bg-white/10 border border-white/5 text-[10px] text-white/80 px-4 py-2 rounded-xl transition-all font-bold tracking-wider uppercase cursor-pointer"
                >
                  {ticketCopied ? (
        ...
        <div className="grid grid-cols-2 gap-2 pt-4 border-t border-white/5">
          ...
          <button
            onClick={() => navigate('/jogos')}
            className="bg-white/10 text-white font-extrabold text-[10px] py-3 rounded-lg flex items-center justify-center gap-1.5 uppercase tracking-widest hover:bg-white/20 transition-all cursor-pointer"
          >
