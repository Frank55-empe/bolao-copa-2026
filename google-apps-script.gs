...
export default function Matches({ user }: { user: UserData }) {
  ...
  // ✅ CORREÇÃO 1: mostrar apenas o jogo ativo escolhido pelo admin
  const activeMatch = matches.find(m => m.id === activeMatchId && m.status !== 'FINISHED');
    ...
    <div className="max-w-2xl mx-auto w-full pb-8">
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
          <h2 className="text-4xl title-display text-transparent bg-clip-text bg-gradient-to-b from-white to-gray-400">Jogos da Copa</h2>
          ...
          >
            ...
            {!activeMatch ? (
              <div className="glass-card p-12 rounded-3xl text-center">
                ...
                <p className="text-sm text-white/30">Aguarde o administrador abrir o próximo jogo.</p>
              </div>
            ) : (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="w-full glass-card p-6 md:p-8 rounded-3xl relative overflow-hidden border-2 border-[#009739] shadow-[0_0_30px_rgba(0,151,57,0.3)] hover:border-[#009739]/80 cursor-pointer hover:scale-[1.005] transition-all"
                onClick={() => navigate(`/palpite/${activeMatch.id}`, { state: activeMatch })}
              >
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6 pt-4">
                  <div className="flex flex-col items-center gap-2 w-full md:w-32">
                    {activeMatch.teamAFlag.startsWith('http') ? (
                      ...
                      <span className="text-5xl md:text-6xl drop-shadow-xl">{activeMatch.teamAFlag}</span>
                    )}
                  ...
                  <div className="flex flex-col items-center gap-2 w-full md:w-32">
                    {activeMatch.teamBFlag.startsWith('http') ? (
                      ...
                      <span className="text-5xl md:text-6xl drop-shadow-xl">{activeMatch.teamBFlag}</span>
                    )}
                ...
                <div className="mt-6 flex justify-center">
                  <span className="inline-flex items-center gap-2 bg-[#009739]/20 border border-[#009739]/40 text-[#009739] text-xs font-black uppercase tracking-widest px-4 py-2 rounded-full">
        ...
        {showRegulamento && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            >
              ...
              <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 text-white/95 text-sm leading-relaxed">
                {rules ? (
