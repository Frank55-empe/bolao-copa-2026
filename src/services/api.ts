...
function cleanObj(obj: any): any {
  ...
  Object.keys(obj).forEach(k => {
    ...
    // Google Sheets às vezes retorna datas como ISO. Normaliza para DD/MM/YYYY
    if (key === 'date' && typeof val === 'string' && (val.includes('T') || val.match(/^\d{4}-\d{2}-\d{2}/))) {
      try {
        const d = new Date(val);
        if (!isNaN(d.getTime())) {
          const day   = String(d.getUTCDate()).padStart(2, '0');
          const month = String(d.getUTCMonth() + 1).padStart(2, '0');
          val = `${day}/${month}/${d.getUTCFullYear()}`;
        }
      } catch { /* mantém original */ }
    }

    // Google Sheets retorna horários como "1899-12-30T22:00:00.000Z" ou similar.
    // Normaliza para HH:MM no horário UTC (que é o valor real do horário)
    if (key === 'time' && typeof val === 'string' && val.includes('T')) {
      try {
        const d = new Date(val);
        if (!isNaN(d.getTime())) {
    ...
    // Normaliza statusPix: planilha antiga pode ter PENDENTE/PAGO (PT) -> PENDING/PAID (EN)
    if (key === 'statusPix' && typeof val === 'string') {
      const v = val.trim().toUpperCase();
      if (v === 'PENDENTE') val = 'PENDING';
      else if (v === 'PAGO') val = 'PAID';
      else val = v;
    }
