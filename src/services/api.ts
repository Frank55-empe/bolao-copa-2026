...
// URL do Google Apps Script - definida via variável de ambiente VITE_SHEETS_API_URL
...
class ApiService {
  ...
  /**
   * Todas as chamadas ao Google Apps Script usam GET com parâmetros na URL.
   * O Apps Script não aceita POST com Content-Type: application/json via browser (CORS).
   * action = nome da ação
   * payload = objeto com os dados, serializado como JSON na URL
   */
  private async fetchSheets(action: string, payload?: any): Promise<any> {
    ...
    try {
      ...
      // Remove possíveis BOM ou caracteres extras antes de parsear
