...
export const SHEETS_API_URL: string =
  ...
  'https://script.google.com/macros/s/AKfycbzmYlVOYPykJJE-kvDuxVWGc3NpoBOfQv6SYNZObl1CBkZnbfWCN2LvCzK1MxIyjtvb/exec';
...
function parseApiDate(raw: any): string {
  if (!raw) return '';
...
function parseApiTime(raw: any): string {
  if (!raw) return '';
  const s = String(raw).trim();
...
// ─── Normaliza status (aceita PT e EN) ───────────────────────────────────────
function parseStatus(raw: any): 'PENDING' | 'FINISHED' {
  const s = String(raw || '').trim().toUpperCase();
  if (s === 'FINISHED' || s === 'ENCERRADO' || s === 'FINALIZADO') return 'FINISHED';
  return 'PENDING';
}
...
function normalizeMatch(m: any): MatchData {
  return {
    ...
    date:         parseApiDate(m.date),
    time:         parseApiTime(m.time),
...
class ApiService {
  ...
  async getMatches(): Promise<MatchData[]> {
    try {
      ...
      if (rows.length > 0) {
        // Normaliza sem filtrar por status — aceita PENDENTE, PENDING, ENCERRADO, etc.
        const normalized = rows
          .map(normalizeMatch)
          .filter(m => m.id && m.teamA && m.teamB);
        if (normalized.length > 0) {
          console.log('[API] getMatches: planilha retornou', normalized.length, 'jogos');
          return normalized; // sempre prefere dados da planilha ao fallback
        }
      }
    } catch (e) {
      console.warn('[API] getMatches falhou, usando fallback:', e);
    }
    console.warn('[API] getMatches: usando dados hardcoded (planilha vazia ou erro)');
