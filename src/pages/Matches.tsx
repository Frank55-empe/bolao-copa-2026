            ...
            {matches.length === 0 ? (
               <div className="glass-card p-12 rounded-3xl text-center">
                ...
                matches.map((match, idx) => {
                  ...
                  const isFeatured = match.id === activeMatchId && !isFinished;
                  return (
                  <motion.div
                    key={match.id}
                    ...
                    className={`w-full glass-card p-6 md:p-8 rounded-3xl relative overflow-hidden transition-all ${
                      isFinished 
                        ? 'border border-white/5 bg-black/25 opacity-70 mb-4' 
                        : isFeatured 
                          ? 'border-2 border-[#009739] shadow-[0_0_30px_rgba(0,151,57,0.3)] mb-6 hover:border-[#009739]/80 cursor-pointer hover:scale-[1.005]' 
                          : 'border border-white/10 mb-4 opacity-80 hover:opacity-100 cursor-pointer hover:scale-[1.005]'
                    }`}
                    onClick={() => {
                      if (!isFinished) {
