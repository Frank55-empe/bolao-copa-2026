...
export default function Matches({ user }: { user: UserData }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const activeId = settingsData.active_match_id || '';
      ...
      // Apenas o jogo ativo escolhido pelo administrador
      const found = matchesData.find((m: MatchData) => String(m.id).trim() === String(activeId).trim()) || null;
