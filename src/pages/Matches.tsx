...
/** Formata data e hora para exibição amigável, aceitando ISO ou DD/MM/AAAA */
function formatMatchDisplay(dateStr: string, timeStr: string): { dateDisplay: string; timeDisplay: string } {
  const parsed = parseMatchDate(dateStr, timeStr);
  if (!parsed) return { dateDisplay: dateStr, timeDisplay: timeStr };
...
/** Retorna true se já passou o prazo (1 hora antes do jogo ou depois) */
function isPredictionLocked(dateStr: string, timeStr: string): boolean {
  const matchDate = parseMatchDate(dateStr, timeStr);
  if (!matchDate) return false;
  const cutoff = new Date(matchDate.getTime() - 60 * 60 * 1000); // 1 hora antes
...
function parseMatchDate(dateStr: string, timeStr: string): Date | null {
  try {
    ...
    // Formato ISO: "2026-06-24T03:00:00.000Z" — vem direto do Google Sheets
    if (dateStr.includes('T') || dateStr.includes('-')) {
      // Tenta parsear a data ISO diretamente
      const isoDate = new Date(dateStr);
      if (!isNaN(isoDate.getTime())) {
        // Converte UTC para horário local de Brasília (UTC-3)
        // e reconstrói como data local sem conversão adicional
        const localDate = new Date(isoDate.getTime());
        return localDate;
      }
      // Fallback: formato YYYY-MM-DD sem hora
      const parts = dateStr.split('T')[0].split('-');
      if (parts.length === 3) {
        const [y, mo, d] = parts.map(Number);
        if (!timeStr) return new Date(y, mo - 1, d, 0, 0);
    ...
    // Formato DD/MM/YYYY (padrão do código)
...
export default function Matches({ user }: { user: UserData }) {
  ...
  const [predictionsLocked, setPredictionsLocked] = useState<boolean>(false);
          ...
          >
            ...
            {!activeMatch ? (
              ...
              (() => {
                ...
                const isAutoLocked = !isFinished && isPredictionLocked(match.date, match.time);
                const isLocked = isAutoLocked || predictionsLocked;
                return (
                  <motion.div
                    key={match.id}
                    ...
                    onClick={() => {
                      if (!isFinished && !isLocked) {
                        // Salva o match no sessionStorage ANTES de navegar
                        // Garante que o Predict encontre os dados mesmo após reload SPA
                        try {
                          sessionStorage.setItem('bolao2026_match_' + match.id, JSON.stringify(match));
                        } catch {}
                  ...
                  >
                    <div className="absolute top-0 right-0 p-4 flex gap-2">
                       {isFinished ? (
                         ...
                         <span className="text-[9px] font-black text-orange-400 bg-orange-500/10 border border-orange-500/20 px-2.5 py-1 rounded-full uppercase tracking-wider">
                           {predictionsLocked ? '🔒 Bolão Pausado' : '🔒 Palpites Fechados'}
                         </span>
                       ) : (
                        ...
                        {isFinished ? (
                          ...
                          <>
                            ...
                            <div className="text-center flex flex-col items-center justify-center">
                              ...
                              {(() => {
                                const { dateDisplay, timeDisplay } = formatMatchDisplay(match.date, match.time);
                                return (
                                  <>
                                    <p className="text-xs font-black text-[#FFCD00]">{dateDisplay} • {timeDisplay}</p>
                    ...
                    {isLocked && !isFinished && (
                      <div className="mt-6 flex justify-center">
                        <div className="inline-flex flex-col items-center gap-1.5 bg-orange-500/10 border border-orange-500/30 px-6 py-3 rounded-2xl text-center">
                          {predictionsLocked ? (
                            <>
                              <span className="text-orange-400 font-black text-sm uppercase tracking-widest">🔒 Bolão Temporariamente Pausado</span>
                              <span className="text-white/50 text-xs font-medium">O administrador pausou os palpites. Aguarde a reabertura.</span>
                            </>
                          ) : (
                            <>
                              <span className="text-orange-400 font-black text-sm uppercase tracking-widest">🔒 Palpites Encerrados</span>
                              <span className="text-white/50 text-xs font-medium">As apostas foram fechadas 1 hora antes do início do jogo.</span>
                            </>
                          )}
                        </div>
                      </div>
