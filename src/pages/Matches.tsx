...
export default function Matches({ user }: { user: UserData }) {
  ...
  // Tick a cada segundo para recalcular status de aposta
  useEffect(() => {
          ...
          >
            ...
            {!activeMatch ? (
              ...
              (() => {
                ...
                const bettingOpen = isBettingOpen(match.date, match.time);
                const canBet = !isFinished && bettingOpen;
                return (
                  <motion.div
                    key={match.id}
                      ...
                      <div className="mt-6">
                        {bettingOpen ? (
                          <>
                            ...
                            {/* Aviso nos últimos 90 min */}
                            {msUntilClose(match.date, match.time) <= 90 * 60_000 && (
                              <div className={`mt-3 flex items-center justify-center gap-2 px-4 py-2.5 rounded-2xl border text-xs font-bold uppercase tracking-widest ${
                                msUntilClose(match.date, match.time) < 15 * 60_000
                                  ? 'bg-red-500/15 border-red-500/40 text-red-300 animate-pulse'
                                  : 'bg-amber-500/10 border-amber-500/30 text-amber-200'
                              }`}>
                                <Lock className="w-3.5 h-3.5 flex-shrink-0" />
                                <span>Apostas fecham em 1h antes do jogo</span>
                              </div>
                            )}
                          </>
                        ) : (
                          <div className="bg-gradient-to-r from-red-900/30 via-red-800/20 to-red-900/30 border-2 border-red-500/40 rounded-2xl p-5 text-center shadow-lg">
                            ...
                            <p className="text-white/70 text-xs leading-relaxed">
                              As apostas foram encerradas <strong className="text-white">{MINUTES_BEFORE_CLOSE} minutos</strong> antes do início.
                              <br /><span className="text-white/40">{match.date} às {match.time}</span>
                            </p>
