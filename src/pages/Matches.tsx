...
// Verifica se o jogo está bloqueado (menos de 1h para início ou já começou)
function isMatchLocked(dateStr: string, timeStr: string): boolean {
  try {
                  ...
                  <motion.div
                    ...
                    className={`w-full glass-card p-6 md:p-8 rounded-3xl relative overflow-hidden transition-all ${
                      isFinished 
                        ...
                        : isMatchLocked(match.date, match.time)
                          ? 'border-2 border-rose-500/40 bg-rose-950/20 cursor-not-allowed'
                          : 'border-2 border-[#009739] shadow-[0_0_30px_rgba(0,151,57,0.3)] hover:border-[#009739]/80 cursor-pointer hover:scale-[1.005]'
                    }`}
                    onClick={() => {
                      if (!isFinished && !isMatchLocked(match.date, match.time)) {
                  ...
                  >
                    ...
                    {!isFinished && (
                      <div className="mt-6 flex justify-center">
                        {isMatchLocked(match.date, match.time) ? (
                          <span className="inline-flex items-center gap-2 bg-rose-500/10 border border-rose-500/30 text-rose-300 font-black text-xs uppercase tracking-widest px-6 py-2.5 rounded-full">
                            🔒 Apostas Encerradas
                          </span>
                        ) : (
