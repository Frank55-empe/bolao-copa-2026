...
export default function Matches({ user }: { user: UserData }) {
  ...
  useEffect(() => {
    ...
    .then(([matchesData, predictionsData, settingsData]) => {
      ...
      // 1. ID exato da planilha (settings.active_match_id)
      // 2. Primeiro jogo não-encerrado da planilha
      // 3. Primeiro jogo da lista (qualquer status)
      // IMPORTANTE: só usa o fallback hardcoded se a planilha não retornar NADA
      ...
      if (!found) {
        // Aceita qualquer status diferente de FINISHED/ENCERRADO/FINALIZADO
        found = matchesData.find((m: MatchData) =>
          m.status !== 'FINISHED'
        ) ?? null;
      }

      // Último recurso: primeiro jogo da lista (mesmo encerrado)
