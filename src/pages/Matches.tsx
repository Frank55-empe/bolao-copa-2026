          ...
          >
            ...
            {!activeMatch ? (
              ...
              (() => {
                ...
                const isLocked = !isFinished && isPredictionLocked(match.date, match.time);
                return (
                  <motion.div
                    key={match.id}
                    ...
                    onClick={() => {
                      if (!isFinished && !isLocked) {
                        // Salva no sessionStorage e localStorage antes de navegar
                        // para garantir que o Predict encontre os dados mesmo após redirect SPA
                        try {
                          const matchJson = JSON.stringify(match);
                          sessionStorage.setItem(MATCH_CACHE_KEY + match.id, matchJson);
                          localStorage.setItem(MATCH_CACHE_KEY + match.id, matchJson);
                        } catch { /* ignore */ }
