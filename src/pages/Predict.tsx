...
import { ArrowLeft, Check, AlertTriangle, BarChart3, Trophy, User, Phone, Calendar, X, Lock, Clock } from 'lucide-react';
import { MatchData, UserData } from '../App';
import { api } from '../services/api';


// ─── Bloqueio de apostas 1h antes do jogo ─────────────────────────────────────
const MINUTES_BEFORE_CLOSE = 60;

function parseMatchDatePredict(dateStr: string, timeStr: string): Date | null {
  try {
...
function isBettingOpenPredict(dateStr: string, timeStr: string): boolean {
  const matchDate = parseMatchDatePredict(dateStr, timeStr);
  if (!matchDate) return true;
  const closeAt = new Date(matchDate.getTime() - MINUTES_BEFORE_CLOSE * 60_000);
  return Date.now() < closeAt.getTime();
}

function msUntilClosePredict(dateStr: string, timeStr: string): number {
  const matchDate = parseMatchDatePredict(dateStr, timeStr);
...
// ─── Tela exibida quando as apostas estão bloqueadas ──────────────────────────
function BettingLockedScreen({ match, onBack }: { match: any; onBack: () => void }) {
  return (
    <div className="max-w-xl mx-auto w-full">
      ...
      <div className="glass-card rounded-3xl p-8 text-center border-2 border-red-500/40 shadow-[0_0_40px_rgba(239,68,68,0.12)]">
        ...
        <h2 className="text-2xl font-black uppercase tracking-widest text-white mb-2">Apostas Encerradas</h2>
        ...
        <div className="bg-black/30 border border-white/10 rounded-2xl p-5 mb-6">
          ...
          <div className="flex items-center justify-center gap-4">
            <div className="flex flex-col items-center gap-1.5">
              <div className="w-12 h-12 rounded-full overflow-hidden border border-white/10 grayscale opacity-60 flex items-center justify-center bg-white/5">
                {match.teamAFlag?.startsWith('http')
                  ? <img src={match.teamAFlag} alt={match.teamA} className="w-full h-full object-cover" />
                  : <span className="text-2xl">{match.teamAFlag}</span>}
              </div>
            ...
            <div className="flex flex-col items-center gap-1.5">
              <div className="w-12 h-12 rounded-full overflow-hidden border border-white/10 grayscale opacity-60 flex items-center justify-center bg-white/5">
                {match.teamBFlag?.startsWith('http')
                  ...
                  : <span className="text-2xl">{match.teamBFlag}</span>}
              </div>
        ...
        <button onClick={onBack} className="w-full bg-white/5 hover:bg-white/10 border border-white/10 text-white font-bold py-3.5 rounded-xl transition-all uppercase tracking-widest text-xs">
          ← Voltar aos Jogos
        </button>
      </div>
    </div>
...
export default function Predict({ user }: { user: UserData }) {
  ...
  // matchData: prioriza location.state (passado via navigate),
  // mas busca na API se state estiver null (ex: reload ou link direto)
  ...
  // Busca o match na API quando location.state não existe
  useEffect(() => {
    if (!matchData && matchId) {
      api.getMatches()
        .then(matches => {
          const found = matches.find((m: MatchData) => m.id === matchId);
          if (found) {
            setMatchData(found);
          } else {
            setFetchError(true);
  ...
  // Reativo: recalcula isBettingOpen a cada segundo
  const [open, setOpen] = React.useState(true);
  React.useEffect(() => {
    if (!matchData) return;
    const check = () => setOpen(isBettingOpenPredict(matchData.date, matchData.time));
    check();
    const id = setInterval(check, 1000);
    return () => clearInterval(id);
  }, [matchData]);

  // Ainda carregando o match
  if (!matchData && !fetchError) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4">
        ...
        <p className="text-white/40 text-sm uppercase tracking-widest font-bold">Carregando jogo...</p>
      </div>
    );
  }

  // Erro ao buscar — volta para jogos
  ...
  // Se apostas fechadas ou jogo finalizado, mostra tela de bloqueio
  if (match.status === 'FINISHED' || !open) {
  ...
  const handleOpenConfirm = (e: React.FormEvent) => {
    e.preventDefault();
    // Barreira de segurança no submit
    if (!isBettingOpenPredict(match.date, match.time)) {
  ...
  const checkAndSubmit = async (skipDuplicateCheck: boolean = false) => {
    // Última barreira — verifica bloqueio exatamente no envio
    if (!isBettingOpenPredict(match.date, match.time)) {
      alert('⏰ As apostas foram encerradas. Tente no próximo jogo!');
      navigate('/jogos');
      return;
    }
    setLoading(true);
  ...
  return (
    <div className="max-w-xl mx-auto w-full">
      ...
      >
        ...
        {msUntilClosePredict(match.date, match.time) <= 90 * 60_000 && isBettingOpenPredict(match.date, match.time) && (
          <div className={`flex items-center justify-center gap-2 text-xs font-bold uppercase tracking-widest px-4 py-2.5 rounded-2xl border mb-6 ${
            msUntilClosePredict(match.date, match.time) < 15 * 60_000
              ? 'bg-red-500/15 border-red-500/40 text-red-300 animate-pulse'
              : 'bg-amber-500/10 border-amber-500/30 text-amber-200'
          }`}>
            <Lock className="w-3.5 h-3.5 flex-shrink-0" />
            <span>Apostas fecham em <strong className="font-mono text-white">1h antes do jogo!</strong></span>
          </div>
        )}
            ...
            >
              ...
              <div className="space-y-3">
                <button 
                  ...
                  disabled={loading}
                  className="w-full bg-gradient-to-r from-[#009739] to-[#00702a] text-white font-bold uppercase tracking-widest py-3.5 rounded-xl hover:scale-[1.02] active:scale-95 transition-all text-xs flex items-center justify-center gap-2 shadow-lg cursor-pointer font-display"
                >
                  {loading ? (
