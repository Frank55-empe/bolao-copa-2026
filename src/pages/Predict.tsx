...
export default function Predict({ user }: { user: UserData }) {
  ...
  useEffect(() => {
    ...
    // Tenta recuperar do sessionStorage OU localStorage (caso de reload / redirect SPA)
    if (matchId) {
      try {
        const cached = sessionStorage.getItem(MATCH_CACHE_KEY + matchId)
          || localStorage.getItem(MATCH_CACHE_KEY + matchId);
        if (cached) {
  ...
  if (!match) {
    return (
      <div className="max-w-xl mx-auto w-full text-center py-20">
        <p className="text-white/50 mb-4">Jogo não encontrado.</p>
        <button onClick={() => navigate('/jogos')} className="bg-[#009739] text-white font-bold py-3 px-8 rounded-xl">
          Voltar aos Jogos
        </button>
      </div>
    );
  ...
  const locked = isPredictionLocked(match.date, match.time);
