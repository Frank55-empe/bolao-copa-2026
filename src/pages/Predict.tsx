...
export default function Predict({ user }: { user: UserData }) {
  ...
  // match vem do location.state (navigate com state)
  // Se não tiver (reload, link direto), busca na API pelo matchId
  const [match, setMatch] = useState<MatchData | null>(
    (location.state as MatchData) || null
  );
  const [loadingMatch, setLoadingMatch] = useState(!(location.state as MatchData));
  ...
  // Busca match na API se não veio pelo state
  useEffect(() => {
    if (!match && matchId) {
      api.getMatches()
        .then(matches => {
          const found = matches.find((m: MatchData) => m.id === matchId) || null;
          setMatch(found);
  ...
  // Enquanto carrega o match
  if (loadingMatch) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        <div className="w-12 h-12 border-4 border-[#009739]/30 border-t-[#009739] rounded-full animate-spin" />
        <p className="text-white/40 text-sm uppercase tracking-widest font-bold">Carregando...</p>
      </div>
    );
  }
  ...
  if (!match) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6">
        <p className="text-white/50 font-bold text-lg">Jogo não encontrado.</p>
        <button
          onClick={() => navigate('/jogos')}
          className="bg-[#009739] text-white font-bold py-3 px-6 rounded-xl uppercase tracking-widest text-sm"
        >
          Voltar aos Jogos
        </button>
      </div>
          ...
          <div className="flex justify-center items-center gap-6 bg-black/20 rounded-3xl p-6 border border-white/5 mb-8 max-w-sm mx-auto shadow-inner">
            <div className="flex flex-col items-center">
              <div className="flex items-center gap-2 md:gap-3">
                ...
                <div className="w-16 h-20 md:w-20 md:h-24 bg-gradient-to-b from-white/10 to-white/5 rounded-2xl flex items-center justify-center text-5xl md:text-6xl font-black text-white border-2 border-white/10 shadow-[inset_0_4px_20px_rgba(0,0,0,0.5)]">{goalsA}</div>
            ...
            <div className="flex flex-col items-center">
              <div className="flex items-center gap-2 md:gap-3">
                ...
                <div className="w-16 h-20 md:w-20 md:h-24 bg-gradient-to-b from-white/10 to-white/5 rounded-2xl flex items-center justify-center text-5xl md:text-6xl font-black text-white border-2 border-white/10 shadow-[inset_0_4px_20px_rgba(0,0,0,0.5)]">{goalsB}</div>
        ...
        {showDuplicateWarning && (
          ...
          >
            <motion.div
              ...
              className="glass-card neon-glow-yellow rounded-3xl p-6 w-full max-w-md"
            >
              ...
              <p className="text-center text-white/70 mb-8 leading-relaxed font-medium">
                {duplicatesCount >= 5 ? (
                  <>O limite para esse palpite foi atingido (<strong className="text-[#FFCD00]">5 participantes</strong>).<br /><br />Escolha outro placar.</>
                ) : (
                  <>Já existem <strong className="text-[#FFCD00] text-lg">{duplicatesCount} participantes</strong> com esse palpite.<br /><br />Deseja continuar? O prêmio será dividido.</>
                )}
              </p>
              <div className="space-y-3">
                {duplicatesCount < 5 && (
        ...
        {showConfirmModal && (
          ...
          >
            <motion.div
              ...
              className="glass-card border-2 border-[#009739]/50 shadow-[0_0_40px_rgba(0,151,57,0.25)] rounded-3xl p-6 md:p-8 w-full max-w-md relative"
            >
