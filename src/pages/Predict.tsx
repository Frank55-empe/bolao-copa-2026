...
export default function Predict({ user }: { user: UserData }) {
  ...
  const [open, setOpen] = React.useState(() => match ? isBettingOpenPredict(match.date, match.time) : true);
  React.useEffect(() => {
    if (!match) return;
    const id = setInterval(() => setOpen(isBettingOpenPredict(match.date, match.time)), 1000);
