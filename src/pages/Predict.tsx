...
export default function Predict({ user }: { user: UserData }) {
  ...
  // Verifica se falta menos de 1 hora para o jogo
  useEffect(() => {
    ...
    const checkDeadline = () => {
      try {
        ...
        const matchTime  = new Date(y, mo - 1, d, h, min, 0);
        const now        = new Date();
    ...
    checkDeadline();
    const t = setInterval(checkDeadline, 1000);
    ...
    <div className="max-w-xl mx-auto w-full">
      ...
      >
        ...
        <form onSubmit={handleOpenConfirm}>
          ...
          {/* Banner de bloqueio */}
          {isClosed && (
            <div className="flex flex-col items-center gap-3 bg-rose-500/10 border-2 border-rose-500/30 rounded-2xl p-6 mb-6 text-center">
              <Lock className="w-8 h-8 text-rose-400" />
              <p className="text-rose-300 font-black uppercase tracking-widest text-sm">Apostas Encerradas!</p>
              <p className="text-white/60 text-xs leading-relaxed">
                {match.date && new Date() < new Date(match.date.split('/').reverse().join('-') + 'T' + match.time) 
                  ? `Faltam apenas ${timeLeft} para o jogo. As apostas encerram 1 hora antes.`
                  : 'Este jogo já começou. Não é mais possível fazer palpites.'}
              </p>
            </div>
          )}
          ...
          <button 
            ...
            className={`w-full font-bold text-lg py-4 rounded-xl flex items-center justify-center gap-2 uppercase tracking-widest transition-all ${isClosed ? "bg-rose-500/20 border border-rose-500/30 text-rose-300 cursor-not-allowed" : "bg-gradient-to-r from-[#009739] to-[#00702a] text-white neon-glow-green hover:scale-[1.02] active:scale-[0.98]"} disabled:opacity-60`}
          >
            {loading ? (
              <div className="w-6 h-6 border-2 border-white/30 border-t-white rounded-full animate-spin" />
            ) : (
              {isClosed ? (<><Lock className="w-5 h-5" /> APOSTAS ENCERRADAS</>) : (<>SALVAR PALPITE <Check className="w-6 h-6" /></>)}
            )}
          </button>
        </form>
      </motion.div>
